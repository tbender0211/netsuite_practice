...
<body header="nlheader" header-height="10%" footer="nlfooter" footer-height="20pt" padding="0.5in 0.5in 0.5in 0.5in" size="Letter">
    ...
    <#if record.item?has_content>
        ...
        <#list record.item as item>
            ...
            <#assign newItemRate = newItemAmount / item.quantity>
                ...
                <#if item.item != "Shipping" && item.item != "US Tariff" && item.item != "Customs Duties" && item.item != "Dealer Commission">
                    <tr>
                ...
                <#elseif (item.item == "Shipping" && record.custbody_fedag_rollshippingcost == false) ||  (item.item == "US Tariff" && record.custbody_fedag_rolltariffcost == false) || (item.item == "Customs Duties" && record.custbody_fedag_rollduties == false) || (item.item == "Dealer Commission" && record.custbody_fedag_rollcommission == false)>
                    <tr>
                        <td align="center" colspan="3" line-height="150%">${item.quantity}</td>
                        <td colspan="12"><span class="itemname">${item.item}</span><br />${item.description}</td>
                        <td colspan="3">${item.options}</td>
    ...
    <table class="total" style="width: 100%; margin-top: 10px;">
        ...
        <tr>
            ...
            <td align="right">
                ${(((record.subtotal?keep_after('$'))?replace(",", ""))?number - record.custbody_fedag_depositamt)?string.currency}
            </td>
        </tr>
